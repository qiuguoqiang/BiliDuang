    ...
    public class DownloadObject
    {
            ...
            for (int i = 0; i < urls.Count; i++)
            {
                try
                {
                    ...
                    if (!single)
                    {
                        ...
                        message = "开始下载";
                        wc.DownloadFileAsync(uri, saveto + "/" + avname + " - " + name + "_" + i.ToString() + ".flv");
                    }
                    else
                    {
                        message = "开始下载";
                        wc.DownloadFileAsync(uri, saveto + "/" + avname + " - " + name + ".flv");
                    }
                }
        ...
        private void ProgressChanged(object sender, DownloadProgressChangedEventArgs e)
        {
            try
            {
                ...
                message = string.Format("正在下载文件，完成进度{0}/{1}(字节)", e.BytesReceived, e.TotalBytesToReceive);
            }
            catch (Exception ex)
            {
                status = -3;
